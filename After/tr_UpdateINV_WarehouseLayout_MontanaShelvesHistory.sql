...
/****** Object:  Trigger [dbo].[tr_UpdateINV_WarehouseLayout_MontanaShelvesHistory]    Script Date: 2026-01-20 12:20:18 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
-- Author:		<Avery Briggs>
-- Create today:	<2026-01-20 12:20:18>
-- Description:	<Maintain History Table>
-- =============================================
CREATE TRIGGER [dbo].[tr_UpdateINV_WarehouseLayout_MontanaShelvesHistory] 
...
BEGIN
	...
	INSERT INTO
        [BWSdb].[dbo].[hist_INV_WarehouseLayout_MontanaShelves]
    (
        [NestLevel],
        [ModifiedID],
        [ModifiedBy],
    ...
    SELECT
        ...
        NULL,
        (CASE
            WHEN ([D].[ShelfSectionID] IS NULL) AND ([C].[ShelfSectionID] IS NOT NULL) THEN 'ShelfSectionID'
            WHEN ([I].[ShelfSectionID] IS NULL) AND ([D].[ShelfSectionID] IS NOT NULL) THEN 'ShelfSectionID'
            WHEN [D].[ShelfSectionID] <> [C].[ShelfSectionID] THEN 'ShelfSectionID'
            ELSE NULL
        END) AS [ModifiedColumn],
        (CASE
            WHEN ([D].[ShelfSectionID] IS NULL) AND ([C].[ShelfSectionID] IS NOT NULL) THEN 'INSERT'
            WHEN ([I].[ShelfSectionID] IS NULL) AND ([D].[ShelfSectionID] IS NOT NULL) THEN 'DELETE'
            WHEN [D].[ShelfSectionID] <> [C].[ShelfSectionID] THEN 'UPDATE'
            ELSE NULL
        END) AS [Modification],
        CAST([D].[ShelfSectionID] AS NVARCHAR(MAX)) AS [ValueBefore],
        CAST([I].[ShelfSectionID] AS NVARCHAR(MAX)) AS [ValueAfter]
    FROM
        [BWSdb].[dbo].[INV_WarehouseLayout_MontanaShelves] [C]
    INNER JOIN
        INSERTED [I]
    ...
    WHERE 
        (CASE
            WHEN ([D].[ShelfSectionID] IS NULL) AND ([C].[ShelfSectionID] IS NOT NULL) THEN 1
            WHEN ([I].[ShelfSectionID] IS NULL) AND ([D].[ShelfSectionID] IS NOT NULL) THEN 1
            WHEN [D].[ShelfSectionID] <> [I].[ShelfSectionID] THEN 1
            ELSE 0
        END) > 0                        
    ...
    SELECT
        ...
        NULL,
        (CASE
            WHEN ([D].[Shelf] IS NULL) AND ([C].[Shelf] IS NOT NULL) THEN 'Shelf'
            WHEN ([I].[Shelf] IS NULL) AND ([D].[Shelf] IS NOT NULL) THEN 'Shelf'
            WHEN [D].[Shelf] <> [C].[Shelf] THEN 'Shelf'
            ELSE NULL
        END) AS [ModifiedColumn],
        (CASE
            WHEN ([D].[Shelf] IS NULL) AND ([C].[Shelf] IS NOT NULL) THEN 'INSERT'
            WHEN ([I].[Shelf] IS NULL) AND ([D].[Shelf] IS NOT NULL) THEN 'DELETE'
            WHEN [D].[Shelf] <> [C].[Shelf] THEN 'UPDATE'
            ELSE NULL
        END) AS [Modification],
        CAST([D].[Shelf] AS NVARCHAR(MAX)) AS [ValueBefore],
        CAST([I].[Shelf] AS NVARCHAR(MAX)) AS [ValueAfter]
    FROM
        [BWSdb].[dbo].[INV_WarehouseLayout_MontanaShelves] [C]
    INNER JOIN
        INSERTED [I]
    ...
    WHERE 
        (CASE
            WHEN ([D].[Shelf] IS NULL) AND ([C].[Shelf] IS NOT NULL) THEN 1
            WHEN ([I].[Shelf] IS NULL) AND ([D].[Shelf] IS NOT NULL) THEN 1
            WHEN [D].[Shelf] <> [I].[Shelf] THEN 1
            ELSE 0
        END) > 0                        
    ...
    SELECT
        ...
        NULL,
        (CASE
            WHEN ([D].[ShelfRow] IS NULL) AND ([C].[ShelfRow] IS NOT NULL) THEN 'ShelfRow'
            WHEN ([I].[ShelfRow] IS NULL) AND ([D].[ShelfRow] IS NOT NULL) THEN 'ShelfRow'
            WHEN [D].[ShelfRow] <> [C].[ShelfRow] THEN 'ShelfRow'
            ELSE NULL
        END) AS [ModifiedColumn],
        (CASE
            WHEN ([D].[ShelfRow] IS NULL) AND ([C].[ShelfRow] IS NOT NULL) THEN 'INSERT'
            WHEN ([I].[ShelfRow] IS NULL) AND ([D].[ShelfRow] IS NOT NULL) THEN 'DELETE'
            WHEN [D].[ShelfRow] <> [C].[ShelfRow] THEN 'UPDATE'
            ELSE NULL
        END) AS [Modification],
        CAST([D].[ShelfRow] AS NVARCHAR(MAX)) AS [ValueBefore],
        CAST([I].[ShelfRow] AS NVARCHAR(MAX)) AS [ValueAfter]
    FROM
        [BWSdb].[dbo].[INV_WarehouseLayout_MontanaShelves] [C]
    INNER JOIN
        INSERTED [I]
    ...
    WHERE 
        (CASE
            WHEN ([D].[ShelfRow] IS NULL) AND ([C].[ShelfRow] IS NOT NULL) THEN 1
            WHEN ([I].[ShelfRow] IS NULL) AND ([D].[ShelfRow] IS NOT NULL) THEN 1
            WHEN [D].[ShelfRow] <> [I].[ShelfRow] THEN 1
            ELSE 0
        END) > 0                        
