...
/****** Object:  View [dbo].[v_PROD_YellowTags]    Script Date: 2026-02-12 2:39:12 PM ******/
SET ANSI_NULLS ON
GO
...
-- 2026-02-12 11:17 - James Crawford - Fixed bug with [Src] sub-select statement where it was grabbing the PurchaseOrder value from [YT] when it should always be coming from [MD].
-- 2026-02-12 14:39 - James Crawford - Reverted PurchaseOrder change from earlier this morning. Found [BWSdb].[dbo].[tr_PopSysproDataPROD_YellowTags] trigger that was missing the PO value criteria change I made here on 2026-01-14.
...
AS
	...
	SELECT
		...
		, [YT].[LastModified]
		, (CASE 
			WHEN [subWipJobAllMat].[Job] IS NOT NULL THEN 0
			ELSE [YT].[Active] END
		) AS [Active]
		, [YT].[DateActive]
		, [YT].[DateInActive]
	...
	LEFT JOIN (
		SELECT 
			...
			[MD].[MReceivedQty],
			[YT].[POOrigDueDate] AS [YT.POOrigDueDate],
			[YT].[POLatestDueDate] AS [YT.POLatestDueDate],
			[YT].[POReceivedQty] as [YT.POReceivedQty],
			ROW_NUMBER() OVER(
				PARTITION BY
					[MD].[MStockCode]
				ORDER BY
