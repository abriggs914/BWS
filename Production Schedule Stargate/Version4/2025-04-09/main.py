...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.settings = {
            ...
            "allowed_to_publish_stg": ctk.BooleanVar(self, value=self.default_allow_publish_stg == "Yes",
                                                     name="ap_stg"),
            "allowed_to_publish_bws": ctk.BooleanVar(self, value=self.default_allow_publish_bws == "Yes",
                                                     name="ap_bws"),
        ...
        # first point of slow data retrieval
        ...
        if self.concats_multi_combobox_orders_stg:
            ...
            self.df_multi_combobox_data_orders_stg = pd.concat(self.concats_multi_combobox_orders_stg,
                                                               ignore_index=True)
        ...
        if tm:
            print(
                f"{self.canvas_width=}, {self.canvas_height=}, {self.canvas_width_scroll_region_bws=}, {self.canvas_height_scroll_region_bws=}")
            print(
    ...
    def update_allow_publish_stg(self, *args):
        ...
        connect(sql, **STARGATE_SQL_CREDS, do_show=tm, do_print=tm)
        self.settings["allowed_to_publish_stg"] = bool(ap_s)
    ...
    def update_allow_publish_bws(self, *args):
        ...
        connect(sql, **STARGATE_SQL_CREDS, do_show=tm, do_print=tm)
        self.settings["allowed_to_publish_bws"] = bool(ap_b)
                ...
                if tile_data:
                    ...
                    if order:
                        ...
                        if kcc:
                            ...
                            bd_s = self.colour_tile_outline_selected.hex_code
                            ou_s = self.width_tile_outline_selected
                            ...
                            is_selected = (date_, line_) in st
                            # bg = bg_s if is_selected else bg
                            # fg = fg_s if is_selected else fg
                            ou = ou_s if is_selected else ou
                            bd = bd_s if is_selected else bd
                            # ft = ft_s if is_selected else ft
    ...
    def get_current_canvas_view(self) -> tuple[float, float, float, float]:
        ...
        if comp_id == COMPANY.BWS.value:
            ...
            srw = self.canvas_width_scroll_region_bws
        else:
        ...
        if tm:
            print(f"A -> {x_1=}, {x_2=}, {y_1=}, {y_2=}, {srw=}, {srh=}")
        x_1 *= srw
        x_2 *= srw
        y_1 *= srh
        y_2 *= srh
        if tm:
            print(f"B -> {x_1=}, {x_2=}, {y_1=}, {y_2=}, {srw=}, {srh=}")
    ...
    def redraw_legend(self):
        ...
        if tm:
            print(f"{x_1=}, {x_2=}, {y_1=}, {y_2=}, {tw=}, {th=}\n{self.get_current_canvas_view()=}")

        cw = self.canvas_width_og
        x2 = x_1 + cw
        ...
        for j, dat in enumerate(row_legend):
            date = self.list_dates[j]
            tile = dat["tile"]
            bw = float(can.itemcget(tile, "width"))
            bbox = can.bbox(tile)
            ...
            isin = False
            if (x_1 <= bx0 <= x_2) or (x_1 <= bx1 <= x_2):
                isin = True
            if tm:
                print(f"{x_1=}, {y_1=}, {x_2=}, {y_2=}, {isin=}, {bbox=}")
            if isin:
                can.tag_raise(tile)
    ...
    def on_left_click_release_calendar(self, event) -> None:
        ...
        # if tm:
        print(f"on_left_click_calendar, {dt=}, {st=}, {date=}, {line=}")
            ...
            if date_line:
                ...
                if self.is_valid_prod_date(date) != "weekend":
                    ...
                    if is_warranty:
                        ...
                        ans = ctk.YES
                        if not pd.isna(est_ad) and (est_ad != unk_val):
                            if not (fd <= est_ad <= ld):
                                if tm:
            ...
            if order is not None:
                ...
                if comp == COMPANY.BWS.value:
                    ...
                    for k in self.info_frame_columns_bws:
                        v = dat_1.get(k, f"'{k}'=?")
                        self.info_frame_bws.change_value(k, v, show_exception=tm)
                    print(f" BWS")
                else:
                    self.btn_if_goto_stg.configure(state=ctk.NORMAL)
                    for k in self.info_frame_columns_stg:
                        v = dat_1.get(k, f"'{k}'=?")
                        self.info_frame_stg.change_value(k, v, show_exception=tm)
                    print(f" STG")
        else:
            self.info_frame_stg.change_value("Quote#", "?", show_exception=tm)
    ...
    def clear_info_frame(self):
        ...
        if comp == COMPANY.BWS.value:
            ...
            for k in self.info_frame_columns_bws:
                self.info_frame_bws.change_value(k, "", show_exception=tm)
        else:
            self.btn_if_goto_stg.configure(state=ctk.DISABLED)
            for k in self.info_frame_columns_stg:
                self.info_frame_stg.change_value(k, "", show_exception=tm)
    ...
    def set_prod_sched_version(self, num: int, date: datetime.datetime):
        ...
        if comp == COMPANY.STG.value:
            ...
            sql = f"UPDATE\n\t[Prod Sched Version#]\nSET\n\t[Version#]={num}\n\t, [VDate] = '{date:%Y-%m-%d %H:%M:%S}'\n;"
            df = connect(sql, **BWS_SQL_CREDS, do_print=tm, do_show=tm)
        if tm:
    ...
    def click_mb_shift_line(self, event=None):
        ...
        if not do_exec:
            self.messagebox(
                title=self.title_application_short,
                message=self.msg_no_movement_non_publish,
                mode="info",
        ...
        def click_submit(*args):
            ...
            if len(data) > 1:
                ...
                for row in data:
                    ...
                    # TODO THIS SECTION DOESNT SUPPORT BWS OPERATIONS YET 20250409
    ...
    def on_closing(self, do_quit: bool = True, do_commit: bool = True) -> None | list:
        ...
        # in_test_mode = self.settings["TEST_MODE"].get()
        print(f"TEST_MODE={'Y' if tm else 'N'} ON_CLOSING")

        if comp == COMPANY.BWS.value:
            print(f"BWS ON_CLOSING")
        ...
        if (date is not None) and (line is not None):
            ...
            if order is not None:
                ...
                for k in info_frame_columns:
                    v = dat_1.get(k, f"'{k}'=?")
                    self.tl_data["tl_qi_info_frame"].change_value(k, v, show_exception=tm)
        else:
            self.tl_data["tl_qi_info_frame"].change_value(self.quote_key("quote"), "?", show_exception=tm)
