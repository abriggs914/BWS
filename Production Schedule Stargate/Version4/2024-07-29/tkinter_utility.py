...
import utility
from utility import grid_cells, clamp_rect, isnumber, alpha_seq, lstindex
...
def is_tk_var_2(var_in, str_var=True, int_var=True, dbl_var=True, bol_var=True, var_var=True):
...
def entry_factory(master, tv_label=None, tv_entry=None, kwargs_label=None, kwargs_entry=None):
	...
	if tv_label is not None and tv_entry is not None:
		res_tv_label = tv_label if is_tk_var_2(tv_label) else tkinter.StringVar(master, value=tv_label)
		res_tv_entry = tv_entry if is_tk_var_2(tv_entry) else tkinter.StringVar(master, value=tv_entry)
...
def text_factory(master, tv_label=None, tv_text=None, kwargs_label=None, kwargs_text=None):
	...
	if tv_label is not None and tv_text is not None:
		res_tv_label = tv_label if is_tk_var_2(tv_label) else tkinter.StringVar(master, value=tv_label)
		res_tv_text = tv_text if is_tk_var_2(tv_text) else tkinter.StringVar(master, value=tv_text)
	elif tv_label is not None:
		res_tv_label = tv_label if is_tk_var_2(tv_label) else tkinter.StringVar(master, value=tv_label)
...
def combo_factory(master, tv_label=None, kwargs_label=None, tv_combo=None, kwargs_combo=None, values=None):
	...
	if tv_label is not None and tv_combo is not None:
		res_tv_label = tv_label if is_tk_var_2(tv_label) else tkinter.StringVar(master, value=tv_label)
		res_tv_combo = tv_combo if is_tk_var_2(tv_combo) else tkinter.StringVar(master, value=tv_combo)
	elif tv_label is not None:
		res_tv_label = tv_label if is_tk_var_2(tv_label) else tkinter.StringVar(master, value=tv_label)
...
def checkbox_factory(master, buttons, default_values=None, kwargs_buttons=None, rtype=list):
	if hasattr(buttons, '__iter__') and buttons:
		...
		for i, btn in enumerate(buttons):
			...
			if isinstance(btn, (list, tuple)) and (len(btn) > 1):
				...
				if len(btn) == 2:
					...
					raise ValueError(
						f"Parameter 'buttons' must be a list of strings indicating button names, or a list of lists or tuples of length 2 or 3 containing a string name and a single callable arg (name, func) or a callable arg and a trace_variable binding (name, func, 'w').")
		...
		if rtype in (list, tuple):
			...
			return {btn if not isinstance(btn, (list, tuple)) else btn[0]: {"btn": c_buttons[i], "var": tv_vars[i]} for
					i, btn, in enumerate(buttons)}
	else:
		raise Exception("Error, must pass a list of buttons.")
...
class TreeviewController(tkinter.Frame):
	...
	def __init__(
		...
		if not is_tk_var_2(self.tv_label):
...
class CustomMessageBox(tkinter.Toplevel):
	...
	def __init__(self, title='Mess', msg='', x=None, y=None, b1='OK', b2='', b3='', b4='', tab_colour="red",
		...
		self.choice = tkinter.StringVar(self, value="") if not is_tk_var_2(answer_handle) else answer_handle
...
class MultiComboBox(tkinter.Frame):
	...
	def __init__(self, master, data, viewable_column_names=None, height_in_rows=10, indexable_column=0, tv_label=None,
		...
		if tv_label is not None and tv_combo is not None:
			self.res_tv_label = tv_label if is_tk_var_2(tv_label) else tkinter.StringVar(self, value=tv_label)
			self.res_tv_entry = tv_combo if is_tk_var_2(tv_combo) else tkinter.StringVar(self, value=tv_combo)
		elif tv_label is not None:
			self.res_tv_label = tv_label if is_tk_var_2(tv_label) else tkinter.StringVar(self, value=tv_label)
...
class ToggleCanvas(tkinter.Canvas):
	...
	def __init__(
		...
		self.colour_background = colour_background if isinstance(colour_background, Colour) else Colour(
			colour_background)
		self.colour_divider = colour_divider if isinstance(colour_divider, Colour) else Colour(colour_divider)
		self.colour_text_option_a = colour_text_option_a if isinstance(colour_text_option_a, Colour) else Colour(
			colour_text_option_a)
		self.colour_text_option_b = colour_text_option_b if isinstance(colour_text_option_b, Colour) else Colour(
			colour_text_option_b)
		self.colour_option_a = colour_option_a if isinstance(colour_option_a, Colour) else (
			Colour(colour_option_a) if colour_option_a is not None else self.colour_background)
		self.colour_option_b = colour_option_b if isinstance(colour_option_b, Colour) else (
			Colour(colour_option_b) if colour_option_b is not None else self.colour_background)
