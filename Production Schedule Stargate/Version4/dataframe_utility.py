...
#     print(f"BEFORE {df['Available Date']=}")
#     for col in df.columns:
#         # if pd.api.types.is_datetime64_any_dtype(df[col]):
#         if is_date_dtype(df, col):
#             print(f"Adjusting '{col}'")
#             df[col] = df[col].apply(lambda x: date_to_datetime(x.date()) if not pd.isnull(x) else None)
#     print(f"AFTER {df['Available Date']=}")
#     print(f"{df['Available Date'].dtypes=}")
#     return df
