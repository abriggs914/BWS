USE [Stargatedb]
GO
/****** Object:  StoredProcedure [dbo].[sp_PDSProductionSchedule_Slots]    Script Date: 2023-09-20 7:40:44 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

-- Version 2023-09-21 1257
...
--BEGIN
	...
	SELECT
		@sd = '2023-10-01',
		@ed = '2023-10-31',
		@dayOffset = 0  -- represents [PDS Subs Master].[ID]
	;
...
--	[PDS Subs Master].[ID] = @dayOffset AND [PDS Subs Master].[Active] = 1
	...
	AND [Prod Lines].[Active] = 1
	and [Date Declined] is null

	
	SELECT 'A', * FROM #ProdSched 
	WHERE [Prod Line] = 'ED1'
	...
	from (select CalendarDate, [Prod Line], LO 
		  ...
		  where WorkDay = 1 AND [Prod Lines].[Active] = 1) as mainsub
	...
	SELECT 'dtProductionScheduleV2' AS [T], * FROM [dtProductionScheduleV2] ORDER BY [SGQuote] DESC
