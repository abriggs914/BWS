...
from tkinter_utility import tkinter, is_tk_var, auto_font, label_factory, calc_geometry_tl

from dateutil.relativedelta import relativedelta
from colour_utility import *
from unit import Unit
from utility import dict_print, lstindex
...
class CalendarSurface(tkinter.Canvas):
    ...
    def __init__(
            ...
            id_monitor_in_use=True,  # this argument will choose largest by default, otherwise use int.
        ...
        self.id_monitor_in_use = id_monitor_in_use
    ...
    def calc_tile_dims(self):
        ...
        for r in range(self.rows + 1):
            ...
            for c in range(self.cols + 1):
                ...
                tile_colour, outline_colour, active_fill_colour, active_outline_colour, font_colour = self.calc_colours(
                    r, c)
                ...
                # def_font = auto_font(def_font, " "*25, width, y2 - y1, min_font_size=4, max_font_size=20)
                for i in range(1, 7):
                    ...
                    row_2.append([wip_x, wip_y, width, th / 6])
            ...
            new_properties.append(tile_detail_row)

        # with open("output_new_props.txt", "w") as f:
        #     f.write(str(new_properties))
        # print(f"CALCtiles={new_properties}")

        return new_properties

    def init_tiles(self) -> list:

        c_tiles = self.calc_tile_dims()
        tw = self.tile_width
        tw_we = self.tile_width_weekend
        th = self.tile_height
        ...
        for r in range(self.rows + 1):
            ...
            for c in range(self.cols + 1):
                ...
                t1_x1, t1_y1 = props["t1_x1"], props["t1_y1"]
                t2_x1, t2_y1 = props["t2_x1"], props["t2_y1"]
                t3_x1, t3_y1 = props["t3_x1"], props["t3_y1"]
                t4_x1, t4_y1 = props["t4_x1"], props["t4_y1"]
                t5_x1, t5_y1 = props["t5_x1"], props["t5_y1"]
                t6_x1, t6_y1 = props["t6_x1"], props["t6_y1"]
    ...
    def resize_canvas(self, width, height):
        ...
        c_dims = self.calc_tile_dims()
        for r in range(self.rows + 1):
            for c in range(self.cols + 1):
                props = self.tile_properties[r][c]
                r_tag = props["tag_rect"]
                dims = c_dims[r][c]
                x1, y1, x2, y2 = dims["x1"], dims["y1"], dims["x2"], dims["y2"]
                # tw, th = dims["tw"], dims["th"]
                # t1_x1, t1_y1 = dims["t1_x1"], dims["t1_y1"]
                # t2_x1, t2_y1 = dims["t2_x1"], dims["t2_y1"]
                # t3_x1, t3_y1 = dims["t3_x1"], dims["t3_y1"]
                # t4_x1, t4_y1 = dims["t4_x1"], dims["t4_y1"]
                # t5_x1, t5_y1 = dims["t5_x1"], dims["t5_y1"]
                # t6_x1, t6_y1 = dims["t6_x1"], dims["t6_y1"]
                # if r < 1 or c < 1:
                #     print(f"{r=}, {c=}, BEFORE={[props['x1'], props['y1'], props['x2'], props['y2']]}")
                self.tile_properties[r][c].update(dims)
                # if r < 1 or c < 1:
                #     print(f"{r=}, {c=}, BEFORE={[props['x1'], props['y1'], props['x2'], props['y2']]}")
                self.coords(r_tag, x1, y1, x2, y2)

                for i in range(1, 7):
                    t_tag = props[f"t{i}_tag"]
                    x1, y1 = dims[f"t{i}_x1"], dims[f"t{i}_y1"]
                    self.coords(t_tag, x1, y1)
    ...
    def populate_units(self, df: pandas.DataFrame) -> None:
        ...
        for row in df.iterrows():
            ...
            if new_line and (new_line not in self.lines):
                print(f"---A")
                cli = self.lines.index(default_line) + 1
                cdi = date_idx
                if cdi:
                    if self.tile_properties[cli][cdi].get("unit") is not None:
                        while self.tile_properties[cli][cdi]["unit"] is not None:
                            if (cli + 1) == len(self.lines):
                                # cannot find an available line on this date
                                print(f"\t{default_line=}")
                                break
                            default_line = self.lines[cli + 1]
                new_line = default_line
                line_idx = self.lines.index(new_line) + 1
                new_unit.job_start_line_v2 = new_line
            elif (lli := lstindex(self.lines, new_line)) != -1:
                print(f"---B {lli=}")
                line_idx = lli + 1
            else:
                print(f"---C nl='{new_line}', lines={self.lines} =={lstindex(self.lines, new_line)}")
            ...
            if (date_idx is not None) and (line_idx is not None):
                # new_unit.placed = True
                # print(f"NEW placing tile! at {new_unit=} {line_idx=}, {date_idx=}, {new_unit.history['_placed']=}")
            ...
            elif (new_date is not None) and (new_line is not None):
