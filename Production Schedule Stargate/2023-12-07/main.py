...
PROD_DATE = datetime.datetime(2023, 12, 7, 12, 15)
PROGRAM_MODE = "LIVE"
# PROGRAM_MODE = "TEST"
# DARK_MODE = True
DARK_MODE = False
    ...
    while error:
        ...
        try:
            ...
            if isinstance(app, App):
                ...
                messagebox.showerror(f"Fatal Error", "An un-recoverable error has occurred and the program cannot continue.\nPlease report this to BWS IT for further investigation.")
                if os.path.exists("C:/Access/STGProdSched/Errors"):
                    n = datetime.datetime.now()
                    error_file = ("0000" + f"{n:%Y}")[-4:]
                    error_file = f"{error_file}" + ("00" + f"{n:%m}")[-2:]
                    error_file = f"{error_file}" + ("00" + f"{n:%d}")[-2:]
                    error_file = f"{error_file}" + ("00" + f"{n:%H}")[-2:]
                    error_file = f"{error_file}" + ("00" + f"{n:%M}")[-2:]
                    error_file = f"{error_file}.json"

                    error_text = {
                        "text": err,
                        "when": f"{n:%Y-%m-%d %H:%H:%S}",
                        "trace": traceback.format_exc()
                    }
                    with open(error_file, "w") as f:
                        f.write(json.dumps(error_text))

                error = False

            print(f"\n{err=}, {m}{traceback.print_exc()=}")
        else:
            # no error, close after single run
            error = False
