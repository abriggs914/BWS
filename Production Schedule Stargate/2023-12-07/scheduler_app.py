...
class App(tkinter.Tk):
    ...
    def __init__(
        ...
        self.tl_please_wait_tv_lbl = None
        self.tl_please_wait_lbl = None
        self.init_please_wait()
        ...
        # self.state("zoomed")
        self.geometry(calc_geometry_tl(1.0, 1.0, largest=self.id_monitor_in_use))
        ...
        self.label_combo_unit_selection = None
        self.tv_combo_unit_selection = None
        self.combo_unit_selection = None
        self.tv_btn_insert_combo_choice = None
        self.button_insert_combo_choice = None
        self.tv_btn_update_changes = None
        self.button_update_changes = None
        self.tv_btn_undo = None
        self.button_undo = None
        self.tv_btn_redo = None
        self.button_redo = None
        self.tv_btn_refresh = None
        self.button_refresh = None
        self.tv_btn_print_schedule = None
        self.btn_print_schedule = None
        self.tv_entry_unit_scroll_search = None
        self.entry_unit_scroll_search = None
        self.tv_label_submit_unit_search = None
        self.button_submit_unit_search = None
        self.tv_label_button_go_to_today = None
        self.button_go_to_today = None
        self.frame_colour_coder = None
        self.tv_label_colour_coder = None
        self.label_colour_coder = None
        self.canv_btn_show_colour_coder = None
        self.line_shifter = None
        self.tv_ls_start_date = None
        self.tv_ls_end_date = None
        self.tv_label_line_shifter = None
        ...
        self.label_testing_frame = None
        self.tv_btn_export_changes = None
        self.button_export_changes = None
        self.debug_tv_show_history = None
        self.debug_show_history = None
        self.tv_label_debug_app_state = None
        self.debug_label_entry_app_state = None
        self.tv_debug_app_state = None
        self.debug_entry_app_state = None
        self.debug_tv_show_scrollregion = None
        self.debug_show_scrollregion = None
        self.toggle_button_sat = None
        self.toggle_button_sun = None
        self.showing_colour_coder = None
        self.showing_line_shifter = None
        self.reset_val_is_configuring = None
        self.id_after_config = None
        self.tv_is_configuring = None
    ...
    def thread_setup(self):
        ...
        t6 = threading.Thread(target=self.init_errors_directory)
        # t7 = threading.Thread(target=self.init_images)
    ...
    def gui_setup(self):
        ...
        # print(f"{list(self.df_production.columns)=}")
        ...
        self.tv_btn_insert_combo_choice, \
            ...
            command=self.click_insert_combo_choice
        )
        ...
        # print(f"{min_date_stg=}, {max_date_stg=}")
        ...
        self.calendar_surface.bind("<MouseWheel>", self.scroll_wheel_canvas)
        ...
        print(f"END GUI PROCESSING")

    def calc_calendar_surface_dims(self):
        return int(self.window_width * self.width_p), int((self.window_height - (self.window_height / 4)) * self.height_p)
    ...
    def scroll_canvas_y(self, *args):
        self.calendar_surface.yview(*args)
        self.re_draw_legend(None)

    def scroll_wheel_canvas(self, event):
        if event.state & 0x1:
            # print(f"shift-scrolling")
            self.calendar_surface.yview('scroll', int(-1 * (event.delta / 120)), 'units')
        else:
            self.xview(event, 'scroll', int(-1 * (event.delta / 120)), 'units')
    ...
    def init_errors_directory(self):
        if not os.path.isdir("C:/Access/STGProdSched/Errors"):
            os.mkdir("C:/Access/STGProdSched/Errors")
        if not os.path.isdir("./STGProdSched/Errors"):
            os.mkdir("./STGProdSched/Errors")
        ...
        if ht_rc:
            if ht_rc[0] != 0 and ht_rc[1] != 0:
                ...
                if self.app_state == "DRAGGING":
                    ...
                    print(f"INVALID STATE {ht_rc=}, {dt_rc=}")
                    # self.calendar_surface.revert_colour(ht_rc)
        else:
        ...
        if text:
            if pass_thru_date is None and (text in self.calendar_surface.units):
                ...
                if handled:
                    ...
                    if date_in is not None:
                        messagebox.showinfo(title="Search Error", message=f"Error, text '{text}' not found associated with any units, Although it is a valid calendar day.")
            ...
            if not handled and (date_in is not None):
                ...
                if date_in in self.calendar_surface.dates_list:
                    ...
                    messagebox.showinfo(title="Search Error", message=f"Error, date '{text}' not found.")
            elif handled:
                pass
            else:
    ...
    def motion_calendar_surface(self, event):
        ...
        if self.app_state == "DRAGGING":
            ...
            x = self.calendar_surface.canvasx(event.x) - (self.calendar_surface.tile_width_stg / 2)
            y = self.calendar_surface.canvasy(event.y) - (self.calendar_surface.tile_height_stg / 2)
            # bbox = [
    ...
    def re_draw_legend(self, event):
        ...
        tw, th, ts = can.tile_width_stg, can.tile_height_stg, can.tile_space
    ...
    def update(self) -> None:
        if self.app_state == "DRAGGING":
            ...
            if dt is None:
                ...
                tw = self.calendar_surface.tile_width_stg
                th = self.calendar_surface.tile_height_stg
    ...
    def init_please_wait(self, title="Please Wait"):
        # print(f"CREATING TOP LEVEL PLEASE WAIT")
        self.tl_please_wait = tkinter.Toplevel(self, name="pls_wait")
        self.tl_please_wait.title(title)
        self.tl_please_wait.geometry(calc_geometry_tl(0.15, 0.12, largest=self.id_monitor_in_use))
        self.tl_please_wait_tv_lbl,\
        self.tl_please_wait_lbl = \
            label_factory(
                self.tl_please_wait,
                tv_label="",
                kwargs_label={
                    "font": ("Arial", 16),
                    "fg": Colour(EMERALD).hex_code
                }
            )
    ...
    def show_please_wait(self, msg="Processing...", animate=True):
        # print(f"SHOWING TOP LEVEL PLEASE WAIT")
        self.tl_please_wait.deiconify()
        self.tl_please_wait_tv_lbl.set(msg)

        if animate:
            ac = msg[-1]
            id_tl_after = None
            def update_lbl():
                cm = self.tl_please_wait_tv_lbl.get().strip()
                i = cm.index(ac)
                c_msg = cm[i:]
                p = ((len(c_msg) % 3) + 1) * ac
                self.tl_please_wait_tv_lbl.set(f"{5 * ' '}{(cm[:i] + p).ljust(18)}")
                id_tl_after = self.tl_please_wait.after(100, update_lbl)
    ...
    def calc_top_frame_height(self):
        return max([
            # self.frame_top_bar.winfo_height() -
            # (self.multi_combo_unit_selection.winfo_height() if self.multi_combo_unit_selection_is_hidden.get() else 0),
            # ((self.multi_combo_unit_selection.winfo_height() + self.multi_combo_unit_selection.res_canvas.height) if not self.multi_combo_unit_selection_is_hidden.get() else 0),
            (self.multi_combo_unit_selection.winfo_height() + self.multi_combo_unit_selection.res_canvas.height),
    ...
    def on_resize_window(self, event):
        ...
        def do_resize():
            ...
            if self.id_after_config:
                self.after_cancel(self.id_after_config)
            ...
            can_w, can_h = int(self.window_width * self.width_p), int(self.window_height * self.height_p)
            f_height = self.calc_top_frame_height()
            # self.frame_top_bar.configure(width=can_w, height=f_height)
            # print(f"{width=}, {f_height=}, {height=}")
            self.frame_top_bar.configure(width=width, height=f_height)
            self.calendar_surface.resize_canvas(*self.calc_calendar_surface_dims())
            self.re_draw_legend(None)
        ...
        def resize_check():
            if self.id_after_config:
                self.after_cancel(self.id_after_config)
            self.id_after_config = self.after(self.tv_is_configuring.get(), do_resize)

        self.tv_is_configuring.set(self.reset_val_is_configuring)
        # self.id_after_config = self.after(self.tv_is_configuring.get(), resize_check)
        resize_check()
