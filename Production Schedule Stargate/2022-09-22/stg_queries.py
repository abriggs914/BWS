...
SQL_VALID_USER_COUNT = {
    "sql": re.sub("\s+", ' ', """
        SELECT * FROM [PDS Valid Updaters] WHERE LOWER([UserName]) = LOWER('{u}');        
    """),
    "database": "Stargatedb",
    "uid": "SGeu1",
    "pwd": "Pupplies-Hagard->Rio0"
...
def check_user(user_in):
    sql = SQL_VALID_USER_COUNT["sql"].format(u=user_in)
    database = SQL_VALID_USER_COUNT["database"]
    uid = SQL_VALID_USER_COUNT["uid"]
    pwd = SQL_VALID_USER_COUNT["pwd"]
