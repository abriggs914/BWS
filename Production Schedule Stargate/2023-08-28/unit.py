...
class Unit:
	...
	def set_wo_num_v2(self, wo_num_v2_in):
		self.history["_wo_num_v2"].append((datetime.datetime.now(), self.gener_id(), int(wo_num_v2_in)))
		self._wo_num_v2 = int(wo_num_v2_in)
	...
	def set_wo_num(self, wo_num_in):
		self.history["_wo_num"].append((datetime.datetime.now(), self.gener_id(), int(wo_num_in)))
		self._wo_num = int(wo_num_in)
	...
	# _WO
	def get_WO(self):
		if self._WO and not isinstance(self._WO, int):
			self.WO = int(self._WO)
		return self._WO
	...
	def set_WO(self, WO_in):
		...
		# print(f">>> WO IN   {WO_in}")
		self.history["_WO"].append((datetime.datetime.now(), self.gener_id(), int(float(WO_in))))
		self._WO = int(float(WO_in))
		# print(f">>> WO IN F {WO_in}")
		# raise ValueError(f"STOPPPP!")
