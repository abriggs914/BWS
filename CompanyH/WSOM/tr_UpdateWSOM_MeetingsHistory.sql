...
/****** Object:  Trigger [dbo].[tr_UpdateWSOM_MeetingsHistory]    Script Date: 2025-01-28 16:21:08 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE TRIGGER [dbo].[tr_UpdateWSOM_MeetingsHistory] 
ON [dbo].[WSOM_Meetings]
...
BEGIN
    ...
    SELECT
        ...
        NULL,
        (CASE
            WHEN ([D].[DateMeeting] IS NULL) AND ([C].[DateMeeting] IS NOT NULL) THEN 'DateMeeting'
            WHEN ([I].[DateMeeting] IS NULL) AND ([D].[DateMeeting] IS NOT NULL) THEN 'DateMeeting'
            WHEN [D].[DateMeeting] <> [C].[DateMeeting] THEN 'DateMeeting'
            ELSE NULL
        END) AS [ModifiedColumn],
        (CASE
            WHEN ([D].[DateMeeting] IS NULL) AND ([C].[DateMeeting] IS NOT NULL) THEN 'INSERT'
            WHEN ([I].[DateMeeting] IS NULL) AND ([D].[DateMeeting] IS NOT NULL) THEN 'DELETE'
            WHEN [D].[DateMeeting] <> [C].[DateMeeting] THEN 'UPDATE'
            ELSE NULL
        END) AS [Modification],
        CAST([D].[DateMeeting] AS NVARCHAR(MAX)) AS [ValueBefore],
        CAST([I].[DateMeeting] AS NVARCHAR(MAX)) AS [ValueAfter]
    FROM
        [BWSdb].[dbo].[WSOM_Meetings] [C]
    INNER JOIN
        INSERTED [I]
    ...
    WHERE 
        (CASE
            WHEN ([D].[DateMeeting] IS NULL) AND ([C].[DateMeeting] IS NOT NULL) THEN 1
            WHEN ([I].[DateMeeting] IS NULL) AND ([D].[DateMeeting] IS NOT NULL) THEN 1
            WHEN [D].[DateMeeting] <> [I].[DateMeeting] THEN 1
            ELSE 0
        END) > 0                        
    ...
    SELECT
        ...
        NULL,
        (CASE
            WHEN ([D].[Attendance] IS NULL) AND ([C].[Attendance] IS NOT NULL) THEN 'Attendance'
            WHEN ([I].[Attendance] IS NULL) AND ([D].[Attendance] IS NOT NULL) THEN 'Attendance'
            WHEN [D].[Attendance] <> [C].[Attendance] THEN 'Attendance'
            ELSE NULL
        END) AS [ModifiedColumn],
        (CASE
            WHEN ([D].[Attendance] IS NULL) AND ([C].[Attendance] IS NOT NULL) THEN 'INSERT'
            WHEN ([I].[Attendance] IS NULL) AND ([D].[Attendance] IS NOT NULL) THEN 'DELETE'
            WHEN [D].[Attendance] <> [C].[Attendance] THEN 'UPDATE'
            ELSE NULL
        END) AS [Modification],
        CAST([D].[Attendance] AS NVARCHAR(MAX)) AS [ValueBefore],
        CAST([I].[Attendance] AS NVARCHAR(MAX)) AS [ValueAfter]
    FROM
        [BWSdb].[dbo].[WSOM_Meetings] [C]
    INNER JOIN
        INSERTED [I]
    ...
    WHERE 
        (CASE
            WHEN ([D].[Attendance] IS NULL) AND ([C].[Attendance] IS NOT NULL) THEN 1
            WHEN ([I].[Attendance] IS NULL) AND ([D].[Attendance] IS NOT NULL) THEN 1
            WHEN [D].[Attendance] <> [I].[Attendance] THEN 1
            ELSE 0
        END) > 0                        
