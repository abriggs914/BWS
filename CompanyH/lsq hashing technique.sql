...
SELECT
	...
	LEN(@txt) AS [LEN(@txt)], 
	(CASE 
		WHEN LEN(@txt) <= 4000 THEN
			HASHBYTES('SHA1', @txt) 
		ELSE (
			CASE
				WHEN (4000 < LEN(@txt)) AND (LEN(@txt) <= 8000) THEN 
					HASHBYTES('SHA1', HASHBYTES('SHA1', LEFT(@txt, 4000)) + HASHBYTES('SHA1', SUBSTRING(@txt, 4000, 4000)))
				ELSE (
					CASE
						WHEN (4000 < LEN(@txt)) AND (LEN(@txt) <= 8000) THEN
							HASHBYTES('SHA1', HASHBYTES('SHA1', LEFT(@txt, 4000)) + HASHBYTES('SHA1', SUBSTRING(@txt, 4000, 4000)) + HASHBYTES('SHA1', SUBSTRING(@txt, 8000, 4000)))
						ELSE
							NULL
						END
					)
...
FROM (
	SELECT
		[Quote#],
		ISNULL([StandardsConcat], '') + '||' + ISNULL([OptionsConcat], '') + '||' + ISNULL([CustomWorkConcat], '')  AS [txt]
	FROM
		AggregatedData
) AS [Src]
...
SELECT * FROM (
	SELECT 
		...
		LEN(ISNULL(StandardsConcat,'')) +
		LEN(ISNULL(OptionsConcat,'')) +
		LEN(ISNULL(CustomWorkConcat,'')) AS TTL
	FROM AggregatedData
) AS [Src]
WHERE
	[TTL] > 4000;
