...
WHILE @i < @c BEGIN
	...
	SELECT @modelno = (select [Model No] from [BWSdb].[dbo].Orders with (nolock) where Quote# = @quote)
	...
	--Drop and create temp table in tmpdb SQL database for faster processing
	IF OBJECT_ID('tempdb..#QuoteNPOs') IS NOT NULL
	...
	--Drop and create temp table in tmpdb SQL database for faster processing
	IF OBJECT_ID('tempdb..#QuoteswithsameNPOs') IS NOT NULL
