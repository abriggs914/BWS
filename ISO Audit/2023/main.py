...
sales_account = None
failed = None
time_to_send_og = 3000
time_to_send_left = None
...
async def send_some_emails(i, recipient):
    print(f"SEND_SOME_EMAILS")
    ...
    mail.Subject = "TEST BWS Customer Satisfaction Survey 2023"
...
def count_down():
    do_loop = False
    for i in range(n_next_recipients):
        if time_to_send_left[i].get() > 0:
            do_loop = True
            window.after(1, lambda: time_to_send_left[i].set(time_to_send_left[i].get() - 1))
        elif time_to_send_left[i].get() == 0:
            # stop thread i
...
def reset_count_downs():
    for i in range(n_next_recipients):
        time_to_send_left[i].set(time_to_send_og)


# async def click_send_emails(event=None):
#
...
#     if not TESTING:
#         tasks = []
#         for i, recipient in enumerate(next_recipients):
#             tasks.append(asyncio.create_task(send_some_emails(i, recipient)))
#         reset_count_downs()
#         count_down()
...
def click_button(event=None):
    ...
    async def await_emails(i, recipient):
        await send_some_emails(i, recipient)
    ...
    if not TESTING:
        tasks = []
        for i, recipient in enumerate(next_recipients):
            print(f"task creation {i+1} for '{recipient}'")
            loop.create_task(await_emails(i, recipient))
        # reset_count_downs()
        # count_down()
...
#
#         window = tkinter.Tk()
#         time_to_send_left = [tkinter.IntVar(value=time_to_send_og) for _ in range(n_next_recipients)]
