...
SELECT
	...
	,(CASE 
		WHEN 
			[StartRuleTimeT] = [StartTime]
			AND [EndRuleTimeT] = [EndTime]
		THEN 'None' 
		WHEN 
			[StartRuleTimeT] < [StartTime]
			AND [EndRuleTimeT] = [EndTime]
		THEN 'Access StartTime is too late' 
		WHEN 
			[StartRuleTimeT] = [StartTime]
			AND [EndRuleTimeT] > [EndTime]
		THEN 'Access EndTime is too Early'
		WHEN 
			[StartRuleTimeT] < [StartTime]
			AND [EndRuleTimeT] > [EndTime]
		THEN 'Access Shift is too short'  
...
FROM (
	SELECT
		...
		,[EndRuleDoW]
		,[EndRuleTimeS]
		,CAST([StartRuleTimeS] AS TIME)AS [StartRuleTimeT]
		,CAST([EndRuleTimeS] AS TIME)AS [EndRuleTimeT]
		,[Duration(m)]
		,[Duration(h)]
		,[AllowLogon]
		,[ShiftIsEnabled]
	...
	FROM (
		...
		ON
			CAST([F].[Emp#] AS NVARCHAR(6)) = [E].[Employee]
