...
IF @inORout = 1 BEGIN
	...
	SET @first_of_shift = (CASE WHEN (SELECT COUNT(*) FROM [ClkTransaction] WHERE [EmployeeNumber] = @empN AND @p_time BETWEEN DATEADD(HOUR, -6, [LoggedOn]) AND [LoggedOn]) = 0 THEN 1 ELSE 0 END)
