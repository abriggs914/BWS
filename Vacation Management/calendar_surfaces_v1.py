...
class AnnualFrameCalendar(FrameCalendar):
    ...
    def draw_grid(self):
        ...
        print(f"{gc=}")
        self.x_range = gc[0][0][0], gc[-1][-1][2]
        self.y_range = gc[0][0][1], gc[-1][-1][3]
        ...
        for i, row in enumerate(gc):
            ...
            for j, dims in enumerate(row):
                ...
                # self.buttons_grid.append(self.canvas_grid.create_image(*dims[:2], image=images[ii]))
                self.buttons_grid.append(self.canvas_grid.create_image(ix, iy, image=self.drop_down_image))
                ...
                self.canvas_grid.tag_bind(self.buttons_grid[-1], "<Button-1>", self.click_canvas_grid_dropdown)
    ...
    def click_canvas_grid_dropdown(self, event):
        print(f"click_canvas_grid_dropdown")
    ...
    def scroll_on_canvas(self, event):
        ...
        b1, b2, b3, b4 = self.canvas_grid.bbox('all')
        print(f"{event=}")
        x, y = self.canvas_grid.canvasx(event.x), self.canvas_grid.canvasy(event.y)
        ...
        for i, tag in enumerate(self.objects_grid):
            c1, c2, c3, c4 = self.canvas_grid.coords(tag)
            print(f"{i=}\t\t{x1=}, {y1=}\t\t{c1=}, {c2=}, {c3=}, {c4=}")
            aaa = clamp(0, c1, b3)
            bbb = clamp(0, c3, b3)
            ccc = clamp(0, c2, b4)
            ddd = clamp(0, c4, b4)
            print(f"\t{aaa=}, {bbb=}, {ccc=}, {ddd=}, {eee=}, {fff=}")
            if aaa <= eee <= bbb and ccc <= fff <= ddd:
                # and aaa <= (eee + self.tile_width) <= bbb and ccc <= (fff + self.tile_height) <= ddd:
                date = self.dates_grid[i]
                r, c = self.i_to_rc(i)
                print(f"\n\n\tTop left tag is {tag}\n\t{date=}\n\t{(r, c)=}")
                self.tv_label_month.set(date.month)
                self.tv_label_year.set(date.year)
                m_idx = date.month - 1
                if (end_of_month(date) - date).days < 6:
                    m_idx += 1
                self.canvas_grid.itemconfig(self.top_left, fill=self.colours[m_idx][("odd" if r % 2 == 0 else "even")])
                self.top_left = tag
                self.canvas_grid.itemconfig(self.top_left, fill="red")
                break
