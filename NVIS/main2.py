...
class NVISGenerator(tkinter.Tk):
    ...
    def generate_NVIS(self):
        ...
        self.top_level_progress = tkinter.Toplevel(self)
        print("here 1")
        self.top_level_progress.geometry(f"{200}x{100}")
        self.progress_bar = ttk.Progressbar(self.top_level_progress, mode="determinate", value=0, maximum=100, length=75)

        def progress():
            print(f"progressing: {self.progress_bar['value']}")
            self.progress_bar["value"] += 1
            self.progress_bar.update()
            if 0 <= self.progress_bar["value"] < 100:
                self.progress_bar.after(1, progress)
            else:
                self.top_level_progress.destroy()

        print("here 2")
        self.progress_bar.pack()
        # progress()
        self.top_level_progress.mainloop()

        sql = ""
        download_thread = AsyncConnect(sql)
        download_thread.start()

        self.monitor(download_thread)


        # self.progress_bar.start(75)
        ...
        try:
            ...
            self.set_status(str(de), is_error=True)
        # self.progress_bar.step(100 - self.progress_bar["value"])
        # self.progress_bar.stop()
        # self.top_level_progress.withdraw()
        self.top_level_progress.destroy()
