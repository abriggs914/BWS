...
class App(tkinter.Tk):
    ...
    def show_processing(self):
        ...
        dims = self.calc_geometry_tl(0.15, 0.08)
    ...
    def update_proc_time(self):
        ns = int(self.tv_lbl_time.get().replace('s', "").replace('Time:', "").strip()) + 1
        self.tv_lbl_time.set(f"{ns} s")
    ...
    async def process_pdf(self, fn, page_idxs, fails, qtys, passes, p_nums, revs, prices, amounts, invoices, orders, q_ids,
                          print_test=False):
                    ...
                    for j, vals in enumerate(values):
                        ...
                        if all([
                            ...
                            if print_test:
                                ...
                                print(f"{qty=}, {part_number=}, {amount=}, {rev=}, {price=}, {invoice_number=}, {order_number=}")
