...
class PDF(FPDF):
            ...
            if not isinstance(v, dict):
                if isinstance(v, list):
                    ...
                    if i == len(v) - 1:
                        is_list = True
                        continue
                raise ValueError("Parameter \"contents\" is not of the correct format.\nNeeds to be a dict of dictionaries")
            if not is_list:
                res.update({k: v})
    ...
    def table(
        ...
        width = w - (2 * left_margin)
        n_space = width / n_cols
        new_col_widths_og = [n_space for k in range(n_cols)]
        new_col_widths = [n_space for k in range(n_cols)]
        if isinstance(col_widths, dict):
            ...
            for k, v in col_widths.items():
                if k in h_names:
                    idx = h_names.index(k)
                    new_col_widths[idx] = width * v
                    v_sums += v
                    affected_cols.append(idx)
                if v_sums > 1:
                    break
            rem = (1 - v_sums) * width
            rem_div = rem / max(1, (n_cols - len(affected_cols)))
            if rem < 0:
                new_col_widths = new_col_widths_og.copy()
            elif rem > 0:
                new_col_widths = [(ncw + rem_div - n_space) if k not in affected_cols else ncw for k, ncw in enumerate(new_col_widths)]

            print("v_sums:", v_sums, "rem:", rem, "rem_div", rem_div, "width", width, "n_space", n_space, "new_col_widths", new_col_widths)
            print("sun_new_cols:", sum(new_col_widths))

        col_widths = new_col_widths.copy()
...
##########################################
        ...
        print("(N x M): ({} x {})".format(n_rows, n_cols))
        # print("(H x W): ({} x {})".format(height, width))
        print("(CH x CW): ({} x {})".format(cell_height, col_widths))
...
if __name__ == "__main__":
    ...
    pdf = PDF(FILE_NAME, orientation='P', unit='mm', format='A4')
