...
-- =============================================
CREATE TRIGGER  [dbo].[tr_LLTUpdateDate]
   ON [LongLeadQuoteDetails]
   --BEFORE
   AFTER
   --INSTEAD OF
   INSERT
...
BEGIN
	...
	IF TRIGGER_NESTLEVEL() < 2 BEGIN
		...
		SELECT @id = i.[LongLeadQuoteDetailsID#] FROM INSERTED i
		...
		IF EXISTS (SELECT * FROM inserted) AND EXISTS (SELECT * FROM deleted) BEGIN
			...
			UPDATE
				[LongLeadQuoteDetails]
			SET
				[LastUpdated] = GETDATE()
			WHERE
				[LongLeadQuoteDetailsID#] = @id
			;
		END
