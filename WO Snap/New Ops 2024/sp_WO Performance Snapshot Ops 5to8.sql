USE [SysproCompanyA]
GO
/****** Object:  StoredProcedure [dbo].[sp_WO Performance Snapshot Ops 4 5]    Script Date: 2024-03-19 1:37:47 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[sp_WO Performance Snapshot Ops 5to8]
...
FROM (
	SELECT
		...
		SUM([5 Total Budgeted Hours]) + SUM([6 Total Budgeted Hours]) + SUM([7 Total Budgeted Hours]) + SUM([8 Total Budgeted Hours]) AS [Total Budgeted Hours],
		SUM([5 Total Budgeted Hours]) + SUM([6 Total Hours Issued]) + SUM([7 Total Hours Issued]) + SUM([8 Total Hours Issued]) AS [Total Hours Issued],
		SUM([5 Hours Over Budget]) + SUM([6 Hours Over Budget]) + SUM([7 Hours Over Budget]) + SUM([8 Hours Over Budget]) AS [Hours Over Budget],
		[Total Defects]
	FROM (
		SELECT
			@WO AS [WO#],
		...
		FROM (
			...
			UNION 
				...
				SELECT
					...
					0 AS [5 Total Hours Over Budget],
