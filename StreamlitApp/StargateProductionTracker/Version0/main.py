...
time_cache_prod_data_by_op_stg = 30*1000  # 30 sec
time_app_refresh = 12*1000  # every 8 seconds
...
for i, row in df_production_data_by_op.iterrows():
    ...
    old = st.session_state.setdefault(COMP, {}).setdefault(job, {}).setdefault("ops_sum", 0)
    new = row[cols_rename["ProgressOps"]]
    changed = new != old
    key_done_showing = f"DS_{i}"
    show_count = st.session_state.setdefault(COMP, {}).setdefault(key_done_showing, 0)
    rsv_show_count = 6
    ...
    if show_count <= -1:
        ...
        st.session_state[COMP].update({key_done_showing: show_count - 1})
        done_showing = False
    ...
    if changed:
        ...
        print(f"{i=}, {job=}")
        print(f"{new=}, {old=}")
        print(f"{done_showing=}, {show_count=}, {changed=}, {key_done_showing=}")
