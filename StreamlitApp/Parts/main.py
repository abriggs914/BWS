...
# Rerence to ensure created file names do not interfere with Windows reserved file structures
_WINDOWS_RESERVED = {
	"CON", "PRN", "AUX", "NUL",
...
def load_drawings_non_stocked():
	pdfs = [f for f in os.listdir(PATH_STOCK_PDFS) if f.lower().endswith(".pdf")]
	df_pdfs = pd.DataFrame({"pdf": pdfs})
	df_pdfs["StockCode"] = df_pdfs["pdf"].apply(lambda s: s.lower().removesuffix(".pdf").upper().strip())
	df_pdfs = df_pdfs.merge(
		df_parts,
		on="StockCode",
		how="left",
		suffixes=("_pdf", "_part")
...
df_drawings_not_parts = load_drawings_non_stocked()
...
elif pills_search_mode == options_pills_search_mode.index(op_search_mode_by_supplier):
	# Supplier
	col_sup: str = "SupShortName"
	lst_suppliers = df_parts[col_sup].dropna().unique().tolist()
	k_multiselect_supplier = "key_multiselect_supplier"
	multiselect_supplier = st.multiselect(
		label="Supplier",
		key=k_multiselect_supplier,
		options=lst_suppliers
	)
	if multiselect_supplier:
		df_supp: pd.DataFrame = df_parts[df_parts[col_sup].isin(multiselect_supplier)]
		stdf_supp = display_df_paginated(
			df_supp,
			"Supplier Parts",
			key="stdf_supp",
			selection_mode="multi-row",
		...
		if not df_sel_parts.empty:
			...
			if isinstance(df_sel_parts, pd.DataFrame):
				if df_sel_parts.empty:
					...
					textbox_stockcode = df_sel_parts.reset_index().loc[0, "StockCode"]
			...
			if parts_data:
				for i, df_parts_ in enumerate(parts_data):
					...
					if st.button(
						...
						stockcode_tree: list = []
						bfs_idx = 0
						stockcode_tree.extend(df_drawings_to_search["StockCode"].dropna().unique().tolist())
						to_visit = stockcode_tree.copy()
						visited = []
						...
						while to_visit:
							...
							if df_sc.empty:
								continue
							# p_sct_idx = stockcode_tree.index(sc)
							ser_sc = df_sc.reset_index().iloc[0]
							f_name = ser_sc["PDF_Listed"]
							if pd.isna(f_name) or (not f_name):
								f_name = ser_sc["PDF_Stock"]
								...
								with open(f_name, "rb") as f_bin:
									...
									if parts:
										...
										if parts.score > 3:
											...
											# stockcode_tree[p_sct_idx] = df_parts_in_sub["StockCode"].dropna().unique().tolist()
											scs_to_add = df_parts_in_sub["StockCode"].dropna().unique().tolist()
											scs_to_add = [sc_ for sc_ in scs_to_add if sc_ not in visited]
											to_visit.extend(scs_to_add)
										else:
...
if (isinstance(textbox_stockcode, str) and textbox_stockcode) or (isinstance(textbox_stockcode, pd.DataFrame) and not textbox_stockcode.empty):
	...
	if (isinstance(textbox_stockcode, pd.DataFrame) and (not textbox_stockcode.empty)) or ((isinstance(textbox_stockcode, str)) and (textbox_stockcode or selectbox_stockcode)):
		...
		if df_pdfs_non_stock_search.shape[0]:
			...
			cols_non_stock = st.columns(2)
			for i, row in df_pdfs_non_stock_search.iterrows():
				sc = row["StockCode"]
				file_local = row["pdf"]
				file_abs = os.path.join(PATH_STOCK_PDFS, file_local)
				with cols_non_stock[0]:
					btn = st.download_button(
						label=f"Download {file_local}",
						data=open(file_abs, "rb").read(),
						file_name=f"{file_local}",
						mime="application/pdf",
						key=f"download_non_stock_pdf_{i=}_{file_local}"
					)
				with cols_non_stock[1]:
					if st.button(
						...
						st.session_state.update({
							k_pills_search_mode: options_pills_search_mode.index(op_search_mode_by_drawing),
