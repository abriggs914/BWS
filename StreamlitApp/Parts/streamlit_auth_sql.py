...
def _log_event(username: str | None, user_id: int | None, success: bool, event_type: str):
	...
	try:
		...
		cur.execute(
			"""
			INSERT INTO [dbo].[ITSTR_AppUserAccessLog] (ID, UserName, Success, EventType, EventUTC, RemoteIP, UserAgent)
			VALUES (?, ?, ?, ?, ?, ?, ?)
			""",
			user_id,
			username,
...
def register_user(username: str, password: str) -> tuple[bool, str]:
	...
	try:
		...
		conn.commit()
		_log_event(username, user_id, True, "register")
		return True, f"User '{username}' registered."
	except pyodbc.IntegrityError:
		conn.rollback()
		_log_event(username, None, False, "register")
...
def login_user(username: str, password: str) -> tuple[bool, str]:
	...
	try:
		...
		if not row:
			conn.rollback()
			_log_event(username, None, False, "login")
			return False, "Invalid username or password."

		user_id, stored_hash_hex, salt_hex, iters, is_active = row
		if not is_active:
			conn.rollback()
			_log_event(username, int(user_id), False, "login")
		...
		conn.commit()
		_log_event(username, int(user_id), True, "login")
...
def change_user_password(username: str, old_password: str, new_password: str) -> tuple[bool, str]:
	...
	try:
		...
		if not row:
			conn.rollback()
			_log_event(username, None, False, "pw_change")
		...
		if not verify_password(old_password, stored_hash_hex, salt_hex, int(iters)):
			conn.rollback()
			_log_event(username, int(user_id), False, "pw_change")
		...
		conn.commit()
		_log_event(username, int(user_id), True, "pw_change")
...
def st_auth(title: str = "Login:") -> bool:
	if st.session_state.get("user") is None:
		if "user" not in st.session_state:
