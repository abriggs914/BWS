...
class CalendarCanvas2(ctk.CTkCanvas):
    ...
    def click_canvas(self, event, i, j):
        ...
        # print(f"{n=}")
        # df_1 = self.ctk_.df_1.loc[(self.ctk_.df_1["DOB"].dt.month == n.month) & (self.ctk_.df_1["DOB"].dt.day == n.day)]
        # df_1 = df_1.sort_values(by=["Team", "PlayerLast", "PlayerFirst"])
        # text = ""
        # for k, row in df_1.iterrows():
...
class TreeviewController(ctk.CTkScrollableFrame):
    ...
    def __init__(
        ...
        #
        # # print(f"--BB {df_1.shape=}\n{self.df_1}")
        # # print(f"{list(df_1.itertuples())=}\n{len(list(df_1.itertuples()))}")
        # # for i, row in df_1.itertuples():
        # # f = list(range(1015))
        ...
        #     # f.remove(i)
        # # print(f"{f=}")
        # # print(f"B {df_1.shape=}")
        # # print(f"{len(list(df_1.iterrows()))=}")
...
class MultiComboBox(ctk.CTkScrollableFrame):
    ...
    def __init__(self, master, data, viewable_column_names=None, height_in_rows=10, indexable_column=0, tv_label=None,
        ...
        # print(f"PRE {self.tree_controller.df_1=}")
        if self.nan_repr is not None:
            self.tree_controller.df = self.tree_controller.df.fillna(self.nan_repr)
        # print(f"POST {self.tree_controller.df_1=}")
    ...
    def add_new_item(self, val, col=None, rest_values=None, rest_tags=None):
        ...
        for df, vals, tags in new_dfs:
            ...
            for i, row in df.iterrows():
                vals_ = vals[i]
                tags_ = tags[i]
                # # for df_, vals_, tags_ in zip(df_1.iterrows(), vals, tags):
