...
def convert_phone_number(phone: str) -> str:
    ...
    try:
        ...
        for i, c in enumerate(["-", "(", ")", "+"]):
            s_phone = s_phone.replace("-", "")
        s_phone = s_phone.strip()

        p1: str = r'(\d{11}\d)'  # with area code and country code
        p2: str = r'(\d{10}\d)'  # just area code
        p3: str = r'(\d{7}\d)'  # just phone number

        if re.match(p1, s_phone):
            return "A"
        elif re.match(p2, s_phone):
            return "B"
        elif re.match(p3, s_phone):
            return "C"
        else:
            return "D"
    ...
    if not check_password():
        ...
        with grid["content_row_0"]:
            ...
            if st.session_state.get("toggle_admin_mode"):
                ...
                list_df = [st.session_state.get("df_itr_customers")[i:i + N_PER_FRAME] for i in range(0, st.session_state.get("df_itr_customers").shape[0], N_PER_FRAME)]
                df_itr_cust_filt = list_df[data_chunk_choice()]
                st.dataframe(df_itr_cust_filt)

                for i, row in df_itr_cust_filt.iterrows():
                    data_grid.append([st.container(border=True)])
                    data_grid[-1].append(data_grid[-1][0].columns(weights))
                    # data_grid.append(st.columns(weights, border=True))
                    name = row["Name"]
                    # print(f"{i=}, {name=}")
                    ...
                    k_phone_number: str = f"phone_number_{i}"
                    ...
                    st.session_state.update({
                        ...
                        k_ext: ext,
                        k_phone_number: cell_phone
                    })

                    # data_grid[i + 1][cols_names.index("Name")].write(name)
                    data_grid[i + 1][1][cols_names.index("Name")].text_input(
                        label="Name",
                        key=k_name,
                    ...
                    data_grid[i + 1][1][cols_names.index("Display Name")].text_input(
                        label="Display Name",
                        key=k_display_name,
                        label_visibility="hidden"
                    )
                    data_grid[i + 1][1][cols_names.index("DoB")].date_input(
                        label="DoB",
                        key=k_dob,
                        label_visibility="hidden",
                        format="YYYY-MM-DD"
                    ...
                    # selectbox
                    data_grid[i + 1][1][cols_names.index("ShirtSize")].selectbox(
                        label="ShirtSize",
                        key=k_shirt_size,
                    ...
                    data_grid[i + 1][1][cols_names.index("Ext")].selectbox(
                        label="Extension",
                        key=k_ext,
                    ...
                    # data_grid[i + 1][1][cols_names.index("Cell")].write(cell_phone)

                    with data_grid[i + 1][1][cols_names.index("Cell")]:
                        place_holder = "xxxxxx " if cell_phone is None else convert_phone_number(cell_phone)
                        event_phone_number = st_phone_number(
                            label="Phone",
                        ...
                        # st.write(cell_phone)
                        # st.write(event_phone_number)

                layout = {'color': "primary",
                          'style': {'margin-top': '10px'}}
                cust_pagination = pagination_component(len(list_df), layout=layout, key="frame_idx")
                st.write(cust_pagination)
