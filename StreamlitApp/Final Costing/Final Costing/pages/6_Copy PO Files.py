...
PAGE_NAME: str = f"{APP_NAME}_copy_po_files"
...
cols_top = st.columns([0.25, 0.65, 0.1])
...
if pills_version == options_pills_version[1]:
	...
	with st.container(border=True):
		...
		file_cols = [col for col in df_parts.columns if
					 col.upper().replace("ALT_", "").split("_")[0].upper() in file_extensions]
		# st.write(file_cols)
		...
		if not df_parts.empty:
			st.info(
				f"*** Please be advised, program output for version 2 (oct2025) is now '{default_output_folder_root}' ***")
			if st.button(
	...
	if (not df.empty) and st.session_state.get("button_run_part_data"):
		...
		pns = set(df["DXF_F"].unique().tolist()).union(
			set(df["DWG_F"].unique().tolist()).union(set(df["PDF_F"].unique().tolist())).union(
				set(df["STP_F"].unique().tolist())))
...
# New Ideas
###########
new_idea_file = r"C:\Access\copy_po_file_ideas_20251030.json"
now = datetime.datetime.now()

st.markdown("### :red[Have an idea for this program?]")
