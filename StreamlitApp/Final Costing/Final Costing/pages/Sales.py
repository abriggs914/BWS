...
def load_sales_orders() -> pd.DataFrame:
	return connect("[SysproCompanyA].[dbo].[v_OpenSalesOrders]")
...
""").strip()
	...
	for q in lq:
		if q not in lst_quotes:
			...
			# update
			sqls.append(
				f"{sql_u_t} [MeetingID] = {m_id}, [DateResolved] = '{date_resolved}', [ResolvedBy] = '{resolved_by}' WHERE [Quote] = '{q}'")
...
def select_quote(*args):
	...
	selected.sort(key=lambda tup: tup[1] if not pd.isna(tup[1]) else now + datetime.timedelta(seconds=-300),
				  reverse=True)
...
def df_to_sortables(df: pd.DataFrame) -> list[dict]:
	...
	# 	col_items = df[col].dropna().astype(str).tolist()
	# 	out.append({"header": str(col), "items": col_items})
	# return out
	out = []
	for col in df.columns:
		items = [
			f'{("" if pd.isna(v) else str(v)).center(16, ' ')}'
			for v in df[col]
		]
		out.append({"header": str(col), "items": items})
	return out
		...
		if list_quotes:
			if (k_df_meeting_quotes not in st.session_state) or (st.session_state.get(k_df_meeting_quotes) is None) or (
				...
				for i, row in df_meeting_quotes.iterrows():
					...
					df_meeting_quotes.loc[i, "Approved"] = not pd.isna(row["DateResolved"]) and not pd.isna(
						row["ResolvedBy"])
			...
			with cont.container(border=1):
				if not quotes_approved_wsom_not_orders.empty:
					...
					btn_set_orders_quotes_approved_from_wsom = st.button(
						...
						on_click=lambda
							lq=quotes_approved_wsom_not_orders[
								"Quote"].values.tolist(): set_orders_quotes_approved_from_wsom(
							lq)
					)
				else:
			...
			with cont:
				...
				if selected_model:
					...
					st.write(
						f"{df_quotes_left_to_review.shape[0]} / {df_model_quotes.shape[0]} quote(s) left to Approve:")
					view_cols = ["Quote", "Class", "Model No", "MeetingID", "IssueDescription", "DateResolved",
								 "ResolutionDetails",
...
elif pills_control == options_pills_control[3]:
	# Sales Orders
	df_open_sales_orders_details: pd.DataFrame = load_sales_orders()
	df_open_sales_orders_details["SalesOrder"] = df_open_sales_orders_details["SalesOrder"].apply(lambda so: so[-6:])
	df_open_sales_orders_details_cbp: pd.DataFrame = df_open_sales_orders_details.loc[
		df_open_sales_orders_details["SOCanBePicked"] == 1
		]
	display_df(
		df_open_sales_orders_details,
		"df_open_sales_orders_details"
	)
	df_open_sales_orders_details_cbp = df_open_sales_orders_details_cbp.sort_values(
		by=["SalesOrder", "StockCode"]
	)
	unique_sales_orders_cpb: pd.Series = pd.Series(
		df_open_sales_orders_details_cbp["SalesOrder"].dropna().unique()).rename("Unique Sales Orders")
	display_df(
		unique_sales_orders_cpb,
		"Unique"
	)
	display_df(
		df_open_sales_orders_details_cbp,
