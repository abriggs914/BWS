...
def calc_price(step_texts):
    ...
    gross = base_price + options + npos
    disc1_v = disc1 if disc1_type == "fixed" else (gross * disc1 * -1 if disc1_type == "percent" else 0)
    disc1_sub = gross + disc1_v
    disc2_v = disc2 if disc2_type == "fixed" else (disc1_sub * disc2 * -1 if disc2_type == "percent" else 0)
    disc2_sub = disc1_sub + disc2_v
    disc3_v = disc3 if disc3_type == "fixed" else (disc2_sub * disc3 * -1 if disc3_type == "percent" else 0)
    disc3_sub = disc2_sub + disc3_v
...
class App(ctk.CTk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.load_data()
        self.df_orders = self.df_orders_stg if self.company == STG else self.df_orders_bws
        self.df_options = self.df_options_stg if self.company == STG else self.df_options_bws
        self.df_npos = self.df_npos_stg if self.company == STG else self.df_npos_bws
        self.df_orders_showable = self.df_orders_showable_stg if self.company == STG else self.df_orders_showable_bws

        self.frame_inputs: ctk. CTkFrame = ctk.CTkFrame(self)
        self.frame_input_discounts: ctk. CTkFrame = ctk.CTkFrame(self)
        self.frame_input_controls: ctk.CTkFrame = ctk.CTkFrame(self)
        ...
        self.grid_args: dict[str: dict[str: Any]] = {
            # self
            "frame_inputs": {r: 0, c: 0, rs: 1, cs: 1, s: ctk.NSEW},
            "frame_input_discounts": {r: 1, c: 0, rs: 1, cs: 1, s: ctk.NSEW},
            "frame_input_controls": {r: 2, c: 0, rs: 1, cs: 1, s: ctk.NSEW},
            "frame_steps": {r: 0, c: 1, rs: 3, cs: 1, s: ctk.NSEW},
        ...
        self.columnconfigure(0, weight=int(0.4*100/2))
        self.columnconfigure(1, weight=int(0.6*100/2))
        self.grid_widgets()
        self.update_us_sale()
    ...
    def click_tl_select(self):
        ...
        if selected:
            ...
            if not df_order.empty:
                ...
                base_price = sr_order["Price"]
                us_sale = sr_order["US Sale"]
                fx_rate = sr_order["ExchangeRate"]
                sum_options = (df_options["Price"] * df_options["Qty"]).sum()
                sum_npos = df_npos["Price"].sum()
                sum_npos_us = df_npos["US Price"].sum()
                disc1 = sr_order.get("Discount1", 0)
                disc2 = sr_order.get("Discount2", 0)
                disc3 = sr_order.get("Discount3", 0)
                program_disc = sr_order.get("Program Discount", 0)
                volume_disc = sr_order.get("Volume Discount", 0)
                # disc1 = disc1 if not pd.isna(disc1) else 0
                # disc2 = disc2 if not pd.isna(disc2) else 0
                # disc3 = disc3 if not pd.isna(disc3) else 0
                disc1_type = sr_order["Discount1_Type"].lower()
                disc2_type = sr_order["Discount2_Type"].lower()
                disc3_type = sr_order["Discount3_Type"].lower()
                if test_prints:
                    print(f"{base_price=}")
                    print(f"{us_sale=}")
                    print(f"{fx_rate=}")
    ...
    def load_data(self):
        ...
        self.df_orders_showable_bws = self.df_orders_bws[self.list_show_cols_bws.keys()].rename(columns=self.list_show_cols_bws)

    def grid_widgets(self, grid: bool = True):
        if grid:
            # self
            self.frame_inputs.grid(**self.grid_args["frame_inputs"])
            self.frame_input_discounts.grid(**self.grid_args["frame_input_discounts"])
            self.frame_input_controls.grid(**self.grid_args["frame_input_controls"])
            ...
            # self
            self.frame_inputs.grid_forget()
            self.frame_input_discounts.grid_forget()
            self.frame_input_controls.grid_forget()
