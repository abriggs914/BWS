...
def top_n_with_other_bin4(
	...
	grouped = df.groupby(group_by_cols, dropna=dropna) if group_by_cols else [(None, df)]
	...
	if split_value and len(column) > 1:
		value_split = final["Value"].str.split(delim, expand=True, regex=False)
		value_split.columns = column  # data_keys[:1] + column + data_keys[1:]
		data_keys = value_split.columns
		final = final.drop(columns="Value").join(value_split)
	...
	if split_group and len(group_by_cols) > 1:
		...
		final = final.drop(columns="Group").join(group_split)
...
display_df(top_n_with_other_bin4(
	df,
	column=[
		clbl_sortG
	],
