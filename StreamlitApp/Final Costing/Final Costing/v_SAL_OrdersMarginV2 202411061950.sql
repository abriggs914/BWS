...
/****** Object:  View [dbo].[v_SAL_OrdersMarginV2]    Script Date: 2024-11-06 7:49:50 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dbo].[v_SAL_OrdersMarginV2]
...
AS
	...
	SELECT
		...
		,1 / (
			CASE WHEN
				(1 - (([Step3].[SalePriceCDN] - [Step3].[TotalCostSoFar]) / (CASE WHEN [Step3].[SalePriceCDN] = 0 THEN 1 ELSE [Step3].[SalePriceCDN] END))) = 0
			THEN 1
			ELSE (1 - (([Step3].[SalePriceCDN] - [Step3].[TotalCostSoFar]) / (CASE WHEN [Step3].[SalePriceCDN] = 0 THEN 1 ELSE [Step3].[SalePriceCDN] END)))
			END
		) AS [RatioSaleToCostCDN]
		,([Step3].[SalePriceCDN] - [Step3].[TotalCostSoFar]) / (CASE WHEN [Step3].[SalePriceCDN] = 0 THEN 1 ELSE [Step3].[SalePriceCDN] END) AS [MarginCDN%]
		,[Step3].[D1]
		,[Step3].[D2]
		,[Step3].[D3]
		,[Step3].[D1CDN]
