...
ALTER PROCEDURE [sp_ITRSendCancellationNotice]

	@itRequestIDIn BIGINT, @who AS NVARCHAR(MAX), @reason AS NVARCHAR(MAX)=NULL
...
AS BEGIN
	...
	SELECT @subject = 'ITR Cancellation #' + RIGHT('000000' + CAST(@itRequestIDIn AS NVARCHAR(MAX)), 6);

	SELECT @body = '<!DOCTYPE html>
<html>

<head>
    <title>IT Request Cancellation Notice</title>
</head>

<body>
	<div>
		<h2>Notice of Cancellation</h2>
		<p>IT Request #' + RIGHT('000000' + CAST(@itRequestIDIn AS NVARCHAR(MAX)), 6) + ' has been cancelled by ''' + @who + '''.</p>
		<p>Reason:</p>' +
		'</p>' + ISNULL(@reason, '-') + '</p>
		...
		<p>Please review this request, and find a way of tying up loose ends before declining.</p>
	</div>'
