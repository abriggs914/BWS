...
DECLARE @SRC TABLE ([Emp#] REAL, [Dept] NVARCHAR(MAX), [NameFromSyspro] NVARCHAR(250), [1st Name] NVARCHAR(250), [2nd Name] NVARCHAR(250), [Email] NVARCHAR(250));
INSERT INTO @SRC 
SELECT DISTINCT
	[Emp#] AS [Emp#],
	[Dept],
...
FROM (
	...
	FROM (
		SELECT
			...
			[1st Name],
			[2nd Name],
			[@SRC].[1st Name] + ' ' + [@SRC].[2nd Name] AS [NameConcat],
			[Email],
