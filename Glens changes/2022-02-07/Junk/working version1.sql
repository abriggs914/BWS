...
--SET @ed1 = '2030-12-31';
	...
	from [BWSdb].[dbo].[v_Dealer Totals Breakdown By Quote And Date]
...
-- Insert statements for procedure here
	...
	DECLARE @dealers_list AS TABLE ([ID] INT IDENTITY(1, 1), [Initials] NVARCHAR(MAX), [CountryID] INT)
	INSERT INTO @dealers_list SELECT * FROM (select Initials, 
					 case when [CURRENT DEALER CDN] = 1 then 0
						  when [CURRENT DEALER US] = 1 then 1
						  else null end as CountryID 
					 from [BWSdb].[dbo].Dealers
					 where (cast([CURRENT DEALER] as int) + cast([CURRENT DEALER CDN] as int) + cast([CURRENT DEALER US] as int)) > 0 AND [Initials] IS NOT NULL
					 group by Initials, [CURRENT DEALER CDN], [CURRENT DEALER US]) AS [A]
	DECLARE @n_dealers AS INT;
	SELECT @n_dealers = COUNT(*) FROM @dealers_list
	INSERT INTO @dealers_list SELECT 'Misc.', 1
	DECLARE @curr_initials AS NVARCHAR(MAX);
	DECLARE @curr_dealer AS INT;

	WHILE @looping_month_start < @looping_month_stop BEGIN

		SET @t_month = DATEADD(MONTH, @looping_month_start, @sd);
		SET @curr_dealer = 0;
		SELECT @curr_initials = [Initials] FROM @dealers_list WHERE [ID] = @curr_dealer

		WHILE @curr_dealer < @n_dealers BEGIN
			...
			--[InvoiceMonth] INT,
			--[InvoiceYear] INT
				select 0, 0, 0, 0, @curr_initials, 0, 'Misc.', 'Misc.', 'Misc.', 'Misc.', 'Misc.', 'Misc.', 0, 0, 0, 0, 0, MONTH(@t_month), YEAR(@t_month)
			...
			-- [Units Sold] float,
				...
				--[InvoiceYear] INT
			INSERT INTO #tmptablePF
				select 0, 0, @curr_initials, 1, 'Misc.', 'Misc.', 'Misc.', 'Misc.', 'Misc.', 'Misc.', 0, MONTH(@t_month), YEAR(@t_month)

			SET @curr_dealer = @curr_dealer + 1
		END
	...
	SELECT 
		...
		,
		(CASE WHEN [PriorSellingPrice] IS NULL THEN 0 ELSE [PriorSellingPrice] END) AS [Parts Selling Price],
		(CASE WHEN [PriorCOGS] IS NULL THEN 0 ELSE [PriorCOGS] END) AS [Parts COGS]
	FROM
	...
	ON
		...
		AND ([SrcA].[InvoiceMonth] = [SrcB].[GlMonth] AND [SrcA].[InvoiceYear] = [SrcB].[GlYear])
	AND
		CAST(CAST([SrcA].[InvoiceYear] AS NVARCHAR(4)) + '-' + CAST([SrcA].[InvoiceMonth] AS NVARCHAR(2)) + '-01' AS DATE) BETWEEN @sd AND @ed
	ORDER BY
