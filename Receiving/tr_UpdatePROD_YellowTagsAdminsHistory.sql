...
/****** Object:  Trigger [dbo].[tr_UpdatePROD_YellowTagsAdminsHistory]    Script Date: 2025-09-08 11:19:17 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE TRIGGER [dbo].[tr_UpdatePROD_YellowTagsAdminsHistory] 
ON [dbo].[PROD_YellowTagsAdmins]
...
BEGIN
	...
	INSERT INTO
        [BWSdb].[dbo].[hist_PROD_YellowTagsAdmins]
    (
        [NestLevel],
        [ModifiedID],
        [ModifiedBy],
    ...
    FROM
        [BWSdb].[dbo].[PROD_YellowTagsAdmins] [C]
    INNER JOIN
        INSERTED [I]
    ON
        [C].[ID] = [I].[ID]
    ...
    SELECT
        ...
        NULL,
        (CASE
            WHEN ([D].[ITRCustomerID] IS NULL) AND ([C].[ITRCustomerID] IS NOT NULL) THEN 'ITRCustomerID'
            WHEN ([I].[ITRCustomerID] IS NULL) AND ([D].[ITRCustomerID] IS NOT NULL) THEN 'ITRCustomerID'
            WHEN [D].[ITRCustomerID] <> [C].[ITRCustomerID] THEN 'ITRCustomerID'
            ELSE NULL
        END) AS [ModifiedColumn],
        (CASE
            WHEN ([D].[ITRCustomerID] IS NULL) AND ([C].[ITRCustomerID] IS NOT NULL) THEN 'INSERT'
            WHEN ([I].[ITRCustomerID] IS NULL) AND ([D].[ITRCustomerID] IS NOT NULL) THEN 'DELETE'
            WHEN [D].[ITRCustomerID] <> [C].[ITRCustomerID] THEN 'UPDATE'
            ELSE NULL
        END) AS [Modification],
        CAST([D].[ITRCustomerID] AS NVARCHAR(MAX)) AS [ValueBefore],
        CAST([I].[ITRCustomerID] AS NVARCHAR(MAX)) AS [ValueAfter]
    FROM
        [BWSdb].[dbo].[PROD_YellowTagsAdmins] [C]
    INNER JOIN
        INSERTED [I]
    ...
    WHERE 
        (CASE
            WHEN ([D].[ITRCustomerID] IS NULL) AND ([C].[ITRCustomerID] IS NOT NULL) THEN 1
            WHEN ([I].[ITRCustomerID] IS NULL) AND ([D].[ITRCustomerID] IS NOT NULL) THEN 1
            WHEN [D].[ITRCustomerID] <> [I].[ITRCustomerID] THEN 1
            ELSE 0
        END) > 0                        
