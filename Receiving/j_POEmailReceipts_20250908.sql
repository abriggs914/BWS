...
DECLARE @email AS NVARCHAR(2048);
DECLARE @subject AS NVARCHAR(2048);
...
WHILE @i < @j BEGIN
	...
	SELECT
		@firstReceived = ISNULL(MIN([FirstDate]), GETDATE()),
		@lastReceived = ISNULL(MAX([LastDate]), GETDATE()),
		@timesReceived = COUNT(*)
	FROM (
	...
	IF (LEN(ISNULL(@poS, '')) > 0) AND (ISNULL(@newDate, @lastDate) > @lastDate) BEGIN
		...
		IF @complete = 0 BEGIN
			SELECT 
				@body = 
					@body 
					+ '</p><h4 style="color:RGB(200, 34, 34)">BACKORDERED PARTS YET TO BE DELIVERED</h4><p>PO ' 
					+ CAST(ISNULL(@po, 0) AS NVARCHAR(255)) 
					+ ' has been received ' 
					+ ISNULL(CAST(@timesReceived AS NVARCHAR(12)), '?') 
					+ ' time(s), between ' 
					+ ISNULL(CAST(CAST(@firstReceived AS DATE) AS NVARCHAR(255)), '?') 
					+ ' and ' 
					+ ISNULL(CAST(CAST(@lastReceived AS DATE) AS NVARCHAR(255)), '?')
		END
		...
		DECLARE @CompleteP NVARCHAR(512);

		SELECT 
			@body = @body + '<table border="1" cellpadding="5" cellspacing="5"><thead><tr><th>StockCode</th><th>JnlDate</th><th>QtyReceived</th><th>PriceReceived</th><th>Reference</th><th>MOrderQty</th><th>MReceivedQty</th><th>Complete</th></tr></thead><tbody>',
			@i = 0,
			@c = COUNT(*)
		FROM 
			@partsData
		...
		WHILE @i < @c BEGIN
			...
			SELECT
				...
				@CompleteP = ISNULL(CAST([Complete] AS NVARCHAR(512)), 'N/A')
			FROM
				@partsData
			WHERE
				[ID] = @i
			...
			SELECT @body = @body + '<tr>'
			SELECT @body = @body + '<td>' + @StockCode + '</td>'
			SELECT @body = @body + '<td>' + @JnlDate + '</td>'
			SELECT @body = @body + '<td>' + @QtyReceived + '</td>'
			SELECT @body = @body + '<td>' + @PriceReceived + '</td>'
			SELECT @body = @body + '<td>' + @Reference + '</td>'
			SELECT @body = @body + '<td>' + @MQtyOrdered + '</td>'
			SELECT @body = @body + '<td>' + @MQtyReceived + '</td>'
			SELECT @body = @body + '<td style="color:RGB(' 
								+ (CASE WHEN ISNULL(@completeP, 0) = 1 THEN '0,186,12' ELSE '186,12,0' END) 
								+ ')">' 
								+ (CASE WHEN ISNULL(@completeP, 0) = 1 THEN 'YES' ELSE 'NO' END)
								+ '</td>'
			SELECT @body = @body + '</tr>'
