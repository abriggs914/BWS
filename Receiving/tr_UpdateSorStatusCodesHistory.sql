...
/****** Object:  Trigger [dbo].[tr_UpdateSorStatusCodesHistory]    Script Date: 2025-09-02 09:49:54 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE TRIGGER [dbo].[tr_UpdateSorStatusCodesHistory] 
...
BEGIN
	...
	INSERT INTO
        [SysproCompanyA].[dbo].[hist_SorStatusCodes]
    (
        [NestLevel],
        [ModifiedID],
        [ModifiedBy],
    ...
    SELECT
        ...
        NULL,
        (CASE
            WHEN ([D].[StatusCode] IS NULL) AND ([C].[StatusCode] IS NOT NULL) THEN 'StatusCode'
            WHEN ([I].[StatusCode] IS NULL) AND ([D].[StatusCode] IS NOT NULL) THEN 'StatusCode'
            WHEN [D].[StatusCode] <> [C].[StatusCode] THEN 'StatusCode'
            ELSE NULL
        END) AS [ModifiedColumn],
        (CASE
            WHEN ([D].[StatusCode] IS NULL) AND ([C].[StatusCode] IS NOT NULL) THEN 'INSERT'
            WHEN ([I].[StatusCode] IS NULL) AND ([D].[StatusCode] IS NOT NULL) THEN 'DELETE'
            WHEN [D].[StatusCode] <> [C].[StatusCode] THEN 'UPDATE'
            ELSE NULL
        END) AS [Modification],
        CAST([D].[StatusCode] AS NVARCHAR(MAX)) AS [ValueBefore],
        CAST([I].[StatusCode] AS NVARCHAR(MAX)) AS [ValueAfter]
    FROM
        [SysproCompanyA].[dbo].[SorStatusCodes] [C]
    INNER JOIN
        INSERTED [I]
    ...
    WHERE 
        (CASE
            WHEN ([D].[StatusCode] IS NULL) AND ([C].[StatusCode] IS NOT NULL) THEN 1
            WHEN ([I].[StatusCode] IS NULL) AND ([D].[StatusCode] IS NOT NULL) THEN 1
            WHEN [D].[StatusCode] <> [I].[StatusCode] THEN 1
            ELSE 0
        END) > 0                        
