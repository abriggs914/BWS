...
/****** Object:  Trigger [dbo].[tr_UpdateINV_BinLocationsHistory]    Script Date: 2025-07-30 14:16:21 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE TRIGGER [dbo].[tr_UpdateINV_BinLocationsHistory] 
...
BEGIN
	...
	INSERT INTO
        [BWSdb].[dbo].[hist_INV_BinLocations]
    (
        [NestLevel],
        [ModifiedID],
        [ModifiedBy],
    ...
    SELECT
        ...
        NULL,
        (CASE
            WHEN ([D].[Bin] IS NULL) AND ([C].[Bin] IS NOT NULL) THEN 'Bin'
            WHEN ([I].[Bin] IS NULL) AND ([D].[Bin] IS NOT NULL) THEN 'Bin'
            WHEN [D].[Bin] <> [C].[Bin] THEN 'Bin'
            ELSE NULL
        END) AS [ModifiedColumn],
        (CASE
            WHEN ([D].[Bin] IS NULL) AND ([C].[Bin] IS NOT NULL) THEN 'INSERT'
            WHEN ([I].[Bin] IS NULL) AND ([D].[Bin] IS NOT NULL) THEN 'DELETE'
            WHEN [D].[Bin] <> [C].[Bin] THEN 'UPDATE'
            ELSE NULL
        END) AS [Modification],
        CAST([D].[Bin] AS NVARCHAR(MAX)) AS [ValueBefore],
        CAST([I].[Bin] AS NVARCHAR(MAX)) AS [ValueAfter]
    FROM
        [BWSdb].[dbo].[INV_BinLocations] [C]
    INNER JOIN
        INSERTED [I]
    ...
    WHERE 
        (CASE
            WHEN ([D].[Bin] IS NULL) AND ([C].[Bin] IS NOT NULL) THEN 1
            WHEN ([I].[Bin] IS NULL) AND ([D].[Bin] IS NOT NULL) THEN 1
            WHEN [D].[Bin] <> [I].[Bin] THEN 1
            ELSE 0
        END) > 0                        
