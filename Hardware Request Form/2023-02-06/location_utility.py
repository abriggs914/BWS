...
def company_from_location(location_in=None, default_address=True):
    """Based on device's GPS location, return the company for that province.
    Pass a geoPy.Location object to bypass async call.
    Pass default_address param as a string representing a default address."""
    try:
        ...
        if not isinstance(default_address, str) or not default_address:
            if default_address:
