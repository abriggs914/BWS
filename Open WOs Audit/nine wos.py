...
DECLARE @i AS BIGINT;
DECLARE @c AS BIGINT;
DECLARE @jobs AS TABLE ([J] NVARCHAR(MAX));
INSERT INTO @jobs ([J]) VALUES
('20049930'),
('20049892'),
('70004475'),
('40000382'),
...
from WipJobAllMat with (nolock)
	...
	left outer join (select WipMaster.Job, 
		...
		where 
			...
			--AND
			(([ActCompleteDate] IS NULL OR [ActCompleteDate] > @ed) OR ([WipMaster].[Job] IN (SELECT [J] FROM @jobs)))
...
--BEGIN
	...
	WHILE @i < @c BEGIN
		...
		where 
			...
			AND (([ActCompleteDate] IS NULL OR [ActCompleteDate] > @ed) OR ([WipMaster].[Job] IN (SELECT [J] FROM @jobs)))
			AND [JobTenderDate] < @ed
		ORDER BY
			[Job]
		;
...
SELECT * FROM @r WHERE [Job] IN (SELECT [J] FROM @jobs)
...
"""
    ...
    if writable:
        if do_BWS:
            wb.save("BWS output ten WOs.xls")
        else:
            wb.save("STG output ten WOs.xls")
