...
DECLARE @quote AS NVARCHAR(MAX) = 'SG101116';
...
FROM
	...
	AND (
		BWSdb_OrdersV2.DealerBranchID = BWSdb_DealersV2_SalesPersonBranch.DealersV2_SPBID
	)
)
LEFT JOIN 
	Leads
...
WHERE (
		(
			(CAST([BWSdb_OrdersV2].[SGQuote] AS NVARCHAR(MAX)))
		...
		OR ((
			...
			= CAST(@quote AS NVARCHAR(MAX))))
		OR ((
			(CAST([BWSdb_OrdersV2].[Serial Number] AS NVARCHAR(MAX)))
			= CAST(@quote AS NVARCHAR(MAX)))
);

--WHERE (
--		(
--			(CStr([BWSdb_OrdersV2].[SGQuote]))
--			= CStr([Forms]![Edit Existing Quote/Order Parameters]![Quote#]))
--		)
--		OR ((
--			(CStr([BWSdb_OrdersV2].[WO#]))
--			= CStr([Forms]![Edit Existing Quote/Order Parameters]![Quote#])))
--		OR ((
--			(CStr([BWSdb_OrdersV2].[Serial Number]))
--			= CStr([Forms]![Edit Existing Quote/Order Parameters]![Quote#]))
--);
