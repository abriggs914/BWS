...
-- =============================================
ALTER TRIGGER [dbo].[tr_AddNoneNPO]
   ON  [dbo].[Orders]
...
BEGIN
	...
	IF TRIGGER_NESTLEVEL() < 2 BEGIN
		...
		DECLARE @quote_s AS NVARCHAR(MAX);
		DECLARE @nc AS INT;
		DECLARE @ds AS BIT;
		
		SELECT @quote = [Quote#] FROM inserted i;
		SELECT @quote_s = CAST(@quote AS NVARCHAR(MAX));
		SELECT @nc = COUNT(*) FROM [Custom Work] WHERE [Quote#] = @quote;
		...
		IF EXISTS (SELECT * FROM inserted) AND EXISTS (SELECT * FROM deleted) BEGIN
			...
			SELECT @body = 'Update on quote=' + @quote_s + '.'
		...
		IF EXISTS (SELECT * FROM inserted) AND NOT EXISTS (SELECT * FROM deleted) BEGIN
			...
			SELECT @body = 'Insert on quote=' + @quote_s + '.'
			SELECT @body = @body + ' Inserted new record'
		...
		IF EXISTS (SELECT * FROM deleted) AND NOT EXISTS (SELECT * FROM inserted) BEGIN 
			...
			SELECT @body = 'Delete on quote=' + @quote_s + '.'
		END

		DECLARE @s AS NVARCHAR(MAX);
		SELECT @s = '';
		SELECT @persons = 'avery.briggs@bwstrailers.com';
		SELECT @subject = 'New Order @quote="' + @quote_s + '"';
