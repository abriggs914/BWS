USE [SysproCompanyA]
GO
/****** Object:  StoredProcedure [dbo].[sp_TopLevelWOReportJamieMultiV2]    Script Date: 2022-04-05 10:02:50 AM ******/
--SET ANSI_NULLS ON
--GO
...
/*********************************************************************************************************************************************************/
/*********************************************************************************************************************************************************/
/*************************************                    Working version 2022-03-28 9:47 AM                 *********************************************/
/*********************************************************************************************************************************************************/
/*********************************************************************************************************************************************************/
/*********************************************************************************************************************************************************/
...
--	[PorMasterDetail]
--ON
--	[FinalA].[WipMasterStockCode] = [PorMasterDetail].[MStockCode]
--GROUP BY
--	[ph]
--	,[Part Category]
--	,[Operation]
...
/*********************************************************************************************************************************************************/
/*********************************************************************************************************************************************************/
/*************************************                    Working version 2022-04-05 10:08 AM                *********************************************/
/*********************************************************************************************************************************************************/
/*********************************************************************************************************************************************************/
/*********************************************************************************************************************************************************/
--
...
SELECT
	...
	,[WipMasterStockCode]
	,[WipMasterStockDescription]
	--,(CASE WHEN ABS([InvWarehouse].[QtyOnHand]) > MAX([InvWarehouse].[QtyOnHand]) THEN MIN([InvWarehouse].[QtyOnHand]) ELSE MAX([InvWarehouse].[QtyOnHand]) END) AS [QtyOnHand]
	,(SUM([InvWarehouse].[QtyOnHand]) / COUNT([InvWarehouse].[QtyOnHand])) AS [QtyOnHand]
	,AVG([QtyIssued]) AS [QtyIssued]
	,AVG([QtyRequired]) AS [QtyRequired]
	,AVG([HrsIssued]) AS [HrsIssued]
	,[Complete]
	--,[InvWarehouse].[Warehouse]
	--,MAX([PurchaseOrder]) AS [PurchaseOrder]
FROM (
	...
	FROM (
		select
			...
			,WipJobAllMat.StockCode AS [AWipMasterStockCode]
			,WipJobAllMat.StockDescription as [AWipMasterStockDescription]
			,SUM([InvWarehouse].[QtyOnHand]) /14 AS [AQtyOnHand]
			,[QtyIssued] AS [AQtyIssued]
			,AVG(CAST(UnitQtyReqd * QtyToMake as decimal(18, 2))) AS AQtyRequired
			,SUM([RunTimeIssued]) AS [AHrsIssued]
			,(CASE WHEN (CAST(ROUND(([UnitQtyReqd]), 0) AS int)) - CAST(ROUND(([QtyIssued]), 0) AS int) <= 0 THEN 'Y' else 'N' END) AS [AComplete]
			--,WipJobAllMat.[Warehouse] AS [AWarehouse]
			--,[WorkCentre]
